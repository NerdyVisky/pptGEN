...
def generate_slide_content(slide_id, arg_topic, subject, book):
    ...
    if(num_of_vis[3] != 0):
        code_snippets = generate_code_snippets(prompts[3], gpt4_model, slide_id)
    ...
    try:
        ...
        print(f"\t🟠 (8/{STEPS}) Error in assembling content for {slide_id}")
        if not os.path.exists("code/buffer/debug"):
            os.mkdir("code/buffer/debug")
        with open(f"code/buffer/debug/{slide_id}.py", 'w') as f:
            json.dump(dict(outline=outline, 
                           instruct_content=instruct_content, 
                           struct_imgs=struct_imgs, 
                           plot_imgs=plot_imgs, 
                           figure_imgs=figure_imgs, 
                           text_content=text_content), f, indent=4)
        final_content = ''
